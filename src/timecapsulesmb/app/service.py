...
class AppOperationError(RuntimeError):
    def __init__(self, message: str, *, debug: object | None = None) -> None:
...
def _config_path(params: dict[str, object]) -> Path | None:
...
def _bool_param(params: dict[str, object], name: str, default: bool = False) -> bool:
...
def _int_param(params: dict[str, object], name: str, default: int) -> int:
    ...
    try:
        ...
        raise AppOperationError(f"{name} must be an integer") from exc
    if parsed < 0:
        raise AppOperationError(f"{name} must be 0 or greater")
    return parsed


def _string_param(params: dict[str, object], name: str, default: str = "") -> str:
    value = params.get(name, default)
    return "" if value is None else str(value)


def _require_string_param(params: dict[str, object], name: str) -> str:
    value = _string_param(params, name).strip()
    if not value:
        raise AppOperationError(f"missing required parameter: {name}")
    return value
...
def configure_operation(params: dict[str, object], sink: EventSink) -> OperationResult:
    ...
    if not host:
        raise AppOperationError("missing required parameter: host")

    resolution_error = ssh_target_link_local_resolution_error(host, ssh_opts)
    if resolution_error is not None:
        raise AppOperationError(resolution_error)
    ...
    if not probe.ssh_port_reachable:
        if not _bool_param(params, "enable_ssh", True):
            raise AppOperationError("SSH is not reachable and enable_ssh is false.")
        ...
        try:
            ...
            raise AppOperationError("The AirPort admin password did not work.", debug=str(exc)) from exc
        except ACPError as exc:
            raise AppOperationError(f"Failed to enable SSH via ACP: {exc}") from exc
        ...
        if not _wait_for_ssh_port(host, timeout_seconds=_int_param(params, "ssh_wait_timeout", 180)):
            raise AppOperationError("SSH did not open after enabling via ACP.")
        sink.stage(operation, "ssh_probe_after_acp")
        probed_state = probe_connection_state(connection)
        probe = probed_state.probe_result

    if not probe.ssh_authenticated:
        raise AppOperationError(probe.error or "The provided AirPort SSH target and password did not work.")

    compatibility = probed_state.compatibility
    if compatibility is not None and not compatibility.supported:
        raise AppOperationError(render_compatibility_message(compatibility))
...
def _require_supported_payload(target, *, allow_unsupported: bool) -> object:
    ...
    if probe_state is None:
        raise AppOperationError("Failed to determine remote device OS compatibility.")
    ...
    if not compatibility.payload_family:
        raise AppOperationError("No deployable payload is available for this detected device.")
...
def deploy_operation(params: dict[str, object], sink: EventSink) -> OperationResult:
    ...
    yes = _bool_param(params, "yes", True)
    ...
    if failures:
        raise AppOperationError("; ".join(failures))
    ...
    if dry_run:
        ...
        if selection.payload_home is None:
            raise AppOperationError(f"MaSt found {len(mast_discovery.volumes)} deployable HFS volume(s), but deploy could not write to any of them.")
    ...
    if is_netbsd4 and not yes:
        raise AppOperationError("NetBSD 4 deploy requires explicit confirmation.")
    ...
    if not yes:
        ...
        raise AppOperationError(f"Deploy requires confirmation to reboot the {device_name}.")
...
def _verify_payload_upload(
    ...
    *,
    ...
    if not verification.ok:
        raise AppOperationError(f"managed payload verification failed at {payload_home.payload_dir}: {verification.detail}")
...
def _verify_runtime(
    ...
    *,
    ...
    if not managed_runtime_ready(verification):
        raise AppOperationError(f"Managed runtime did not become ready. {verification.detail.strip()}".strip())
...
def _request_reboot_and_wait(
    ...
    *,
    ...
    if not wait_for_ssh_state_conn(connection, expected_up=False, timeout_seconds=down_timeout_seconds):
        raise AppOperationError(reboot_no_down_message)
    ...
    if not wait_for_ssh_state_conn(connection, expected_up=True, timeout_seconds=up_timeout_seconds):
        raise AppOperationError(REBOOT_UP_TIMEOUT_MESSAGE)
...
def activate_operation(params: dict[str, object], sink: EventSink) -> OperationResult:
    ...
    if not is_netbsd4_payload_family(compatibility.payload_family):
        raise AppOperationError("activate is only supported for NetBSD4 AirPort storage devices; use deploy for persistent NetBSD6 installs.")
    sink.stage(operation, "build_activation_plan")
    plan = build_netbsd4_activation_plan()
    if dry_run:
        return OperationResult(True, _jsonable(plan))
    if not yes:
        raise AppOperationError("NetBSD4 activation requires explicit confirmation.")
    connection = target.connection
...
def uninstall_operation(params: dict[str, object], sink: EventSink) -> OperationResult:
    ...
    if not yes:
        raise AppOperationError("Uninstall requires explicit confirmation to reboot.")
    _request_reboot_and_wait(
        operation,
        sink,
        connection,
    ...
    for line in render_post_uninstall_verification(verification):
        sink.log(operation, line)
    if not verification:
        raise AppOperationError("Managed TimeCapsuleSMB files are still present after reboot.")
...
def fsck_operation(params: dict[str, object], sink: EventSink) -> OperationResult:
    ...
    if not yes:
        raise AppOperationError("fsck requires explicit confirmation.")
    sink.stage(operation, "load_config")
    config = load_env_config(env_path=_config_path(params))
    sink.stage(operation, "resolve_connection")
    connection = resolve_env_connection(config, allow_empty_password=True)
...
def repair_xattrs_operation(params: dict[str, object], sink: EventSink) -> OperationResult:
    ...
    dry_run = _bool_param(params, "dry_run")
    yes = _bool_param(params, "yes")
    if not dry_run and not yes:
        raise AppOperationError("repair-xattrs requires dry_run or explicit confirmation.")
    if sys.platform != "darwin":
        raise AppOperationError("repair-xattrs must be run on macOS because it uses xattr/chflags on the mounted SMB share.")
    ...
    with redirect_stdout(output):
        try:
            rc = repair_xattrs_cli.run_repair(args, context, config)
        except SystemExit as exc:
            message = system_exit_message(exc) or "repair-xattrs failed"
            raise AppOperationError(message) from exc
    for line in output.getvalue().splitlines():
        sink.log(operation, line)
    return OperationResult(rc == 0, {"returncode": rc, "telemetry_result": context.result, "error": context.error})
...
def run_api_request(request: dict[str, object], sink: EventSink) -> int:
    ...
    if not operation:
        sink.error("api", "missing required field: operation")
        return 1
    if not isinstance(params, dict):
        sink.error(operation, "params must be a JSON object")
        return 1
    handler = OPERATIONS.get(operation)
    if handler is None:
        sink.error(operation, f"unknown operation: {operation}", debug={"known_operations": sorted(OPERATIONS)})
        return 1
    try:
        result = handler(params, sink)
    except AppOperationError as exc:
        sink.error(operation, str(exc), debug=redact(exc.debug) if exc.debug is not None else None)
        return 1
    except (SystemExit, KeyboardInterrupt):
        raise
    except Exception as exc:
        sink.error(operation, f"{type(exc).__name__}: {exc}")
        return 1
