...
tc_init_runtime_env() {
    ...
    SMB_DEADTIME=${SMB_DEADTIME:-60}
    NBNS_ENABLED=${NBNS_ENABLED:-0}
    RSYNC_ENABLED=${RSYNC_ENABLED:-0}
    TC_SMBD_DISK_LOGGING_ENABLED=${SMBD_DEBUG_LOGGING:-0}

    TC_SMB_DEADTIME=$(tc_sanitize_unsigned_integer "$SMB_DEADTIME" 60)
    if [ "$TC_SMB_DEADTIME" != "$SMB_DEADTIME" ]; then
        tc_add_runtime_env_warning "runtime config: invalid SMB_DEADTIME=$SMB_DEADTIME; using 60"
    fi
    SMB_DEADTIME=$TC_SMB_DEADTIME
