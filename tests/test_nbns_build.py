...
class NbnsBuildWrapperTests(unittest.TestCase):
    ...
    def test_netbsd7_nbns_build_uses_sysroot_static_gc_and_strips_output(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            env, log, gcc_args, strip_args = self.env_for(Path(tmp), triple="arm--netbsdelf")

            result = self.run_wrapper("nbns.sh", env)
            ...
            self.assertTrue((Path(tmp) / "stage" / "nbns-advertiser.stripped").exists())
            self.assertIn("SDK_FAMILY=netbsd7", log.read_text())

    def test_netbsd4le_nbns_wrapper_uses_little_endian_lane_without_sysroot(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            env, log, gcc_args, _strip_args = self.env_for(Path(tmp), triple="arm--netbsdelf")

            result = self.run_wrapper("nbnsoldle.sh", env)
            ...
            self.assertIn("TRIPLE=arm--netbsdelf", log.read_text())
            self.assertIn("SDK_FAMILY=netbsd4", log.read_text())

    def test_nbns_build_fails_before_mutation_when_toolchain_is_missing(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            env = os.environ.copy()
            env.update({
                ...
                "BUILD_SRC": str(root / "src"),
                "NBNS_STAGE": str(root / "stage"),
                "NBNS_LOG": str(root / "nbns.log"),
            })
            ...
            self.assertFalse((root / "nbns.log").exists())
