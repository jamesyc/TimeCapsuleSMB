...
class DeployModuleTests(unittest.TestCase):
    ...
    def test_render_start_script_includes_device_model_flag(self) -> None:
        ...
        self.assertNotIn('--adisk-share "$SMB_SHARE_NAME"', rendered)
        self.assertNotIn('--adisk-disk-key "$ADISK_DISK_KEY"', rendered)
        self.assertNotIn('--adisk-uuid "$ADISK_UUID"', rendered)
        self.assertNotIn('--adisk-sys-wama "$iface_mac"', rendered)
        ...
        self.assertNotIn("ether[[:space:]]", rendered)
        self.assertNotIn("address[[:space:]]", rendered)
