...
pid_t test_fork(void) {
    ...
    collector = fork();
    if (collector > 0 && once("cancel_after_fork")) telemetry_stop = 1;
    return collector;
}
...
int test_setpgid(pid_t pid, pid_t group) {
    if (once("parent_group")) { errno = EPERM; return -1; }
    if (getpid() != owner && !strcmp(scenario, "child_group")) { errno = EPERM; return -1; }
