...
int main(int argc, char **argv) {
    ...
    plan.status.validated = 1; strcpy(plan.id.netbios, "TIMECAPSULE"); strcpy(plan.id.instance, "Caf\303\251 Capsule");
    ...
    file = fopen(source, "w"); if (!file) return 3; fputs("fake-smbd", file); fclose(file); chmod(source, 0755);
    return tc_samba_prepare(&config, &plan, &shares) == 0 ? 0 : 4;
}
